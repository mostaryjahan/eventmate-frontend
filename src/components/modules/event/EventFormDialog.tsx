...
import { createEvent, updateHostedEvent } from "@/services/host/hostedEventManagement";
import { getAllTypes } from "@/services/admin/eventTypeManagement";
import { useActionState, useEffect, useRef, useState } from "react";import { toast } from "sonner";
...
const EventFormDialog = ({ open, onClose, onSuccess, event }: IEventFormDialogProps) => {
  ...
  const formActionFn = isEditMode
    ? async (prevState: any, formData: FormData) => {
        if (!event?.id) return { success: false, message: "Event ID is missing for update." };
  ...
  useEffect(() => {
    if (state?.success) {
      toast.success(state.message || (isEditMode ? "Event updated successfully" : "Event created successfully"));
      if (!isEditMode) formRef.current?.reset(); 
      onClose();
      onSuccess();
    } else if (state?.success === false) {
      const errorMsg = state.errors && typeof state.errors === 'object'
        ? Object.values(state.errors).flat().join(", ") 
        : state.message || (isEditMode ? "Failed to update event" : "Failed to create event");
        ...
        <form ref={formRef} action={formAction} className="space-y-4" onSubmit={(e) => {
          ...
          <FieldGroup className="grid grid-cols-2 gap-4">
            ...
            <Field>
              ...
              <Select value={typeId} onValueChange={setTypeId} required name="typeId">
              ...
              {/* Hidden input to ensure typeId is submitted */}
            ...
            <Field>
              ...
              <Input id="dateTime" name="dateTime" type="datetime-local" required 
                defaultValue={event ? new Date(event.dateTime).toISOString().slice(0, 16) : ""}
              />
              <InputFieldError field="dateTime" state={state} />
            </Field>
            <Field>
              <FieldLabel htmlFor="location">Location</FieldLabel>
              <Input id="location" name="location" type="text" required defaultValue={event?.location} />
              <InputFieldError field="location" state={state} />
            </Field>
           
            <Field>
              <FieldLabel htmlFor="minParticipants">Min Participants</FieldLabel>
              <Input id="minParticipants" name="minParticipants" type="number" required defaultValue={event?.minParticipants} />
              <InputFieldError field="minParticipants" state={state} />
            </Field>
            <Field>
              <FieldLabel htmlFor="maxParticipants">Max Participants</FieldLabel>
              <Input id="maxParticipants" name="maxParticipants" type="number" required defaultValue={event?.maxParticipants} />
              <InputFieldError field="maxParticipants" state={state} />
            </Field>
             <Field>
              <FieldLabel htmlFor="joiningFee">Joining Fee</FieldLabel>
              <Input id="joiningFee" name="joiningFee" type="number" step="0.01" required defaultValue={event?.joiningFee} />
